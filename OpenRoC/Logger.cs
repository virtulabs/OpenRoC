namespace oroc
{
	...
	internal class Log
	{
		...
		{
			Logger.Instance.DebugFormat(fmt, args);
		}
		...
		{
			Logger.Instance.WarnFormat(fmt, args);
		}
		...
		{
			Logger.Instance.ErrorFormat(fmt, args);
		}
		...
		{
			Logger.Instance.InfoFormat(fmt, args);
		}
	}
	...
	internal class Logger
	{
		...
		public static void Configure(Form owner, RichTextBox logBox)
		{
			...
			try
			{ phoenix_root = XElement.Load(config_file.FullName); }
			catch (Exception ex)
			{
		...
		internal class TextBoxAppender : AppenderSkeleton
		{
			...
			private static Color GetLevelColor(Level level)
			{
				...
				{
					return level == Level.Warn ? Color.Blue : Color.Black;
				}
			}
		}
	}
